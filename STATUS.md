...
**Última atualização:** 2026-04-24 (pós-decisão arquitetural: cadeia de custódia por agente MP)
**Próxima revisão sugerida:** ao re-executar `validate_raw_05.py` sob os novos sha256 do reingest, ao estender o validador para B-jurisprudencia (schema §4.4/§4.4.1), ou ao iniciar Fase 1
...
| 0.5 — Landing zone `raw/` + sidecar | ✅ **concluída para A-normativas (commit 3081647, 2026-04-24)** — FASE 2 reingest prospectivo sob regime LFS (`raw/A-normativas/**/*.html filter=lfs diff=lfs merge=lfs -text`) executada com sucesso: 10 HTMLs coletados em 2026-04-24 por agente MP (`baixado_por: pbm_s`), sidecares v1.2 atualizados (`sha256` novo + `etag_http` + `last_modified_http` + `baixado_em` + `baixado_por`), invariante `sha256(disco) == OID(LFS)` validada nos 10, `validate_raw_05.py` rodou **10/10 OK em modo=direto, mojibake 0.0000%, zero artefatos** (CSV em `_AGENTS/validation-reports/2026-04-24-etapa05.csv`). Cadeia de custódia reancorada no ato de coleta documentado por agente público do MP (CPP 158-A Lei 13.964/19; LC 75/93 art. 8º II; Lei 8.625/93 art. 26 I b; CNJ Res. 615/2025; Lei 9.610/98 art. 8º IV) — fundamentação registrada no bloco `reingest_2026_04_24.fundamento_cadeia_custodia` de cada sidecar. Anomalia material isolada em `lei-14188-2021.html`: delta_bytes -1.559 (-8,53%) vs. coleta anterior — recompilação editorial do Planalto, não rotação F5, documentada em `reingest_2026_04_24.anomalia_recompilacao`. B-jurisprudencia (2 pilotos): íntegros desde o primeiro commit (regime LFS prospectivo desde design). | estender `validate_raw_05.py` para schema §4.4/§4.4.1 e rodar sobre B-jurisprudencia; investigar anomalia lei-14188-2021 em Etapa 1 (diff semântico markdown canônico); adensar C/D/E/F/G/H |
...
**Inflexão epistêmica de 2026-04-24:** diagnóstico do F5 Advanced WAF/CSPM no Planalto (nonce + token rotativos por request) e da impossibilidade prática de reprodutibilidade byte-a-byte perpétua com a fonte desencadearam reancoragem formal da cadeia de custódia de `raw/` no ato de coleta documentado por agente público do MP. `raw/` deixa de ser espelho contrafactual do servidor remoto e passa a ser conjunto de snapshots atestados por coleta documentada. O sidecar v1.2 ganha peso normativo central (não meramente descritivo) nos campos `baixado_em` e `baixado_por`. Fundamento: CPP art. 158-A (Lei 13.964/19); LC 75/93 art. 8º II; Lei 8.625/93 (LONMP) art. 26 I b; CNJ Res. 615/2025; Lei 9.610/98 art. 8º IV. Consequência imediata: a proposta de camada `raw-canonical/` com stripping F5 e campo `sha256_canonical` no sidecar é abandonada — complexidade injustificada que a moldura jurídica correta dissolve. Detalhamento completo: `PLANO-INGESTAO.md §Reingest-2026-04-23` e `_AGENTS/AGENTS.md §CADEIA DE CUSTÓDIA — Decisão arquitetural 2026-04-24`.
...
2. ✅ **Rodado sobre os 10 documentos de A pós-FASE 2 reingest (2026-04-24)** — resultado: `ok=10 warning=0 blocked=0`. Todos os sha256 bateram em `modo=direto` (OID LFS == sha256 disco == sidecar, invariante forense íntegra). Mojibake 0.0000% em todos; zero artefatos. CSV em `_AGENTS/validation-reports/2026-04-24-etapa05.csv`.
3. ⏳ **Pendente — emitir `inbox/<id>.md` com `encoding_validated: true`** (requer Etapa 1: conversão HTML→MD via iconv CP1252 + strip HTML) para então iniciar indução de vocabulário. **Atenção em Etapa 1**: `lei-14188-2021.html` apresenta delta_bytes -1.559 (-8,53%) entre coleta anterior e coleta 2026-04-24 — suspeita de recompilação editorial do Planalto. Validar via diff semântico no markdown canônico antes de inferir alteração material do texto normativo (ver `reingest_2026_04_24.anomalia_recompilacao` no sidecar).
...
|---|---|---|
| `_AGENTS/AGENTS.md` | Contrato operacional do pipeline; carregado em toda sessão Cowork | 2026-04-24 (+ §CADEIA DE CUSTÓDIA — decisão arquitetural: ancoragem por agente MP) |
...
| `PLANO-INGESTAO.md` | Plano completo de 5 fases | 2026-04-24 (+ §Reingest-2026-04-23 Fase 1 — cadeia de custódia por agente MP) |
| `STATUS.md` | **este arquivo** — painel de bordo | 2026-04-24 (Fase 0.5 ✅ A-normativas fechada em 3081647; cadeia de custódia por agente MP; item 9 reenquadrado) |
...
9. **Fidelidade byte-a-byte em raw/A-normativas — REFUTADA em 2026-04-23 (ver Calibração #3 em `_AGENTS/AGENTS.md`)** → a predição arquitetural de que a rede tri-modo cobriria divergência cross-OS foi empiricamente refutada. O experimento `lfs-migrate-a-normativas-essay` disparou o smudge filter (`* text=auto eol=lf`, linha 9 do `.gitattributes`) que reescreveu o working tree de 8 dos 10 HTMLs, substituindo bytes mixed-CRLF originais por LF-puros. Mesmo após rollback completo do experimento (`git reset --hard origin/main`, sem LFS ativo), o validador reproduz 8/10 BLOCK com `direto == lf` — porque o próprio reset reaplica o smudge. Consequência: o baseline 10/10 OK de 2026-04-23 (commit 2679d8b) era artefato transitório do worktree local pré-rebuild, não propriedade do repositório. Qualquer clone fresco em Linux (ou em Windows com `core.autocrlf=false`) produz 8/10 BLOCK imediatamente. **A cadeia de custódia byte-a-byte declarada em sidecar v1.2 §4.4.1 para esses oito nunca viveu no repo — viveu apenas no worktree local do Pedro.** Os dois OK (`lei-12737-2012.html`, `lei-13718-2018.html`) são imunes porque eram `w/lf` já no download original. **Próxima operação:** reingest dos 10 HTMLs com pattern LFS prospectivo (ver `PLANO-INGESTAO.md §Reingest-2026-04-23`). Reingest é a única remediação possível; nenhuma ferramenta de history rewrite recupera bytes já consumidos pela text conversion. **Nota (2026-04-24):** o diagnóstico que seguia este item — "cadeia de custódia byte-a-byte nunca viveu no repo" — foi reenquadrado pela decisão arquitetural de 2026-04-24. A impossibilidade de reprodutibilidade byte-a-byte com a fonte é estrutural (F5 Advanced WAF/CSPM injeta nonce + token por request, não defeito do pipeline), e a cadeia de custódia reancora-se no ato de coleta documentado pelo agente MP (CPP 158-A; LC 75/93 art. 8º II). O reingest permanece necessário como higiene de repositório (regime LFS prospectivo correto + sidecares com sha256 atestando os snapshots capturados), mas não é mais remediação forense — é fechamento operacional.
